...
- **`PipelineStep`** -- interface with `name: string` and `execute(ctx): Promise<StepOutput>`. Each step returns `{ status?: "continue" | "skip" | "halt" }`.
...
backend/src/ai/extractors/
  ...
  invoice/
    pipeline/
      ...
      invoiceAfterOcrPipeline.ts  #   Factory: stages 1-7 (pre-engine)
      invoicePostEnginePipeline.ts#   Factory: stages 8-15 (post-engine)
    ...
    learning/                     #   Vendor learning and template storage
      extractionLearningStore.ts
      extractionMappingService.ts
      vendorTemplateStore.ts
      vendorFingerprint.ts
    confidenceScoring/            #   Field-level confidence scoring
      FieldConfidenceScorer.ts
  bank/                           # Bank statement extraction pipeline
    pipeline/
      contextKeys.ts
      bankPipelineFactory.ts
      steps/
        NormalizeTransactionsStep.ts
        DeduplicateTransactionsStep.ts
        PersistBankStatementStep.ts
    BankStatementDocumentDefinition.ts
    BankStatementExtractionPipeline.ts
  shared/                         # Cross-document-type utilities
    provenanceNormalization.ts
```
...
### Post-engine pipeline (stages 8-15)

Built by `createInvoicePostEnginePipeline()`. Runs after the SLM returns structured fields.

| # | Step | Source |
|---|------|--------|
| 8 | MergeBaselineWithSlmStep | invoice/pipeline/steps/ |
| 9 | RecoverOcrFieldsStep | invoice/pipeline/steps/ |
| 10 | ValidateFieldsStep | invoice/pipeline/steps/ |
| 11 | ComputeFieldDiagnosticsStep | invoice/pipeline/steps/ |
| 12 | EnrichComplianceStep | invoice/pipeline/steps/ |
| 13 | AssessConfidenceStep | invoice/pipeline/steps/ |
| 14 | ResolveProvenanceStep | invoice/pipeline/steps/ |
| 15 | BuildExtractionResultStep | invoice/pipeline/steps/ |
...
1. Create a class implementing `PipelineStep` with a descriptive `name`.
