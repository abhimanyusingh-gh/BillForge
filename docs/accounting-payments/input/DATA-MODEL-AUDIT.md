...
- **Fields** (heavily nested, ~80+ leaf fields):
  ...
  - **Nested `extraction`**: `source`, `strategy`, `invoiceType`, `classification` (invoiceType, category, tdsSection), `fieldConfidence` (Map), `fieldProvenance` (Map of provenance sub-schema), `lineItemProvenance[]`, `fieldOverlayPaths` (Map, retained for forward compat on legacy invoices; no longer written by the ingestion pipeline now that field highlights are rendered client-side as DOM boxes)
